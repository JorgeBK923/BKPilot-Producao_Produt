...
> **PRE-SPRINT TECNICA ENCERRADA em 2026-05-25.** Gate 0 do BKPilot-SkillRunner APROVADO COM RESSALVAS (T01-T24, 24/24 tarefas). Detalhe operacional e backlog da Sprint 1 vivem no repo `BKPilot-SkillRunner` (`HANDOFF.md` + `docs/maia/10-memoria/memoria-gate0-2026-05-25.md`, espelhada em `docs/maia/10-memoria/` deste hub). Proxima etapa = Sprint 1 (Engine real reutilizavel), que ABRE por `02-maia-especificacao` (formalizar Gate 1) ANTES de codar - regra dura do owner (2026-05-25): nunca codar sem spec/objetivo.
...
**Proxima skill recomendada:** `02-maia-especificacao` - Sprint 1 (formalizar Gate 1 = criterios de aceite mensuraveis, analogo a G0-1..G0-10, cobrindo: converter 12 skills web, CursorLLM real, meta paridade >= 95%, saldar D1-D8). REGRA DURA do owner: nunca codar sem spec/objetivo. So apos spec+planejamento -> `06-maia-implementacao` T01 (`convert-skill.ts`, Codex/GPT-5.3; validacao Gemini 3.1 Pro).
